...
// ─────────────────────────────────────────────────────────────────────────────
// NOTAS_VENDA — /v1/venda/notas-fiscais
// NF-e / NFC-e de saída (classificacao = VENDA).
// Mesma estrutura NotaFiscal da compra, separada por domínio.
// Itens em `nota-fiscal-venda-itens.ts`.
// ─────────────────────────────────────────────────────────────────────────────
...
export const notasVenda = sqliteTable(
  'notas_venda',
  {
  ...
  (t) => ({
    idxChaveNfe:    index('idx_nf_venda_chave_nfe').on(t.chaveDaNfe),
    idxDataEmissao: index('idx_nf_venda_data_emissao').on(t.dataEmissao),
    idxSituacao:    index('idx_nf_venda_situacao').on(t.situacao),
    idxLojaId:      index('idx_nf_venda_loja_id').on(t.lojaId),
    idxClienteId:   index('idx_nf_venda_cliente_id').on(t.clienteId),
    idxOperacao:    index('idx_nf_venda_operacao_id').on(t.operacaoId),
  }),
);

export type NotaVenda    = typeof notasVenda.$inferSelect;
export type NewNotaVenda = typeof notasVenda.$inferInsert;
