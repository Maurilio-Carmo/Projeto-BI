...
import { notasCompra } from './notas-compra';

// ─────────────────────────────────────────────────────────────────────────────
// NOTA_COMPRA_ITENS
// Array `itens` — linhas de produto da Nota Fiscal de Compra.
// ─────────────────────────────────────────────────────────────────────────────
...
export const notasCompraItens = sqliteTable(
  'notas_compra_itens',
  {
    id: integer('id').primaryKey({ autoIncrement: true }),

    notaCompraId: integer('nota_compra_id')
      .notNull()
      .references(() => notasCompra.id, { onDelete: 'cascade' }),
  ...
  (t) => ({
    idxNotaCompra: index('idx_nf_compra_it_nota_id').on(t.notaCompraId),
    idxProduto:    index('idx_nf_compra_it_produto_id').on(t.produtoId),
    idxNcm:        index('idx_nf_compra_it_ncm').on(t.ncm),
  }),
);

export type NotaCompraItem    = typeof notasCompraItens.$inferSelect;
export type NewNotaCompraItem = typeof notasCompraItens.$inferInsert;
